...
export default {
  ...
  range(min: number, max: number, isEqual: number) {
    return {
      validator(r: any, v: any, callback: any) {
        if (isEqual) {
          min <= v && v <= max ? callback() : callback(new Error(t(`取值范围 ${min} ~ ${max}`)))
        } else {
          min < v && v < max ? callback() : callback(new Error(t(`取值范围在${min} ~ ${max} 之间`)))
        }
      }
    }
  },
  ...
  maxFloat(len = 2) {
    return {
      pattern: new RegExp(`^-?(([1-9][0-9]*)|(([0]\\.\\d{1,2}|[1-9][0-9]*\\.\\d{1,${len}})))$`),
  ...
  length(min: number, max: number) {
    return {
      validator(r: any, v: any, c: any) {
        v += ''
        v.length < min || v.length > max ? c(new Error(t(`字符区间${min}~${max}`))) : c()
      }
    }
  },
  ...
  // 不包含特殊字符 | & # [ ]
  noSpecial() {
    return { pattern: /^((?![\|\&\#\[\]]).)*$/, message: t('不可包含特殊字符 | & # [ ]') }
  },
  // 必须包含验证
  withIn(v: string) {
  ...
  account() {
    ...
    return { pattern: reg, message: t(`只可包含 字母，数字，下划线，减号`), trigger: 'blur' }
  },
  // （字母，数字，下划线）
  nCode() {
    const reg = new RegExp(`^[a-zA-Z0-9_]*$`)
    return { pattern: reg, message: t(`只可包含 字母，数字，下划线`), trigger: 'blur' }
  },
  // (小写字母，下划线)
  lowercaseCode() {
    const reg = new RegExp(`^[a-z_]*$`)
    return { pattern: reg, message: t(`只可包含 小写字母，下划线`), trigger: 'blur' }
  },
  // 只能是字母或则字母加数字(以字母开头)
  onlyLetter() {
    return {
  ...
  // 手机号
  mobile() {
    return { pattern: /^1[345789]\d{9}$/, message: t('请正确输入手机号码'), trigger: 'blur' }
  },
  mobileName() {
    return {
  ...
  telephone() {
    return {
      pattern: /(^(0\d{2})-(\d{8})$)|(^(0\d{3})-(\d{7,8})$)|(^(0\d{2})-(\d{8})-(\d+)$)|(^(0\d{3})-(\d{7})-(\d+)$)/,
      message: t('请正确输入电话号码'),
      trigger: 'blur'
    }
  },
